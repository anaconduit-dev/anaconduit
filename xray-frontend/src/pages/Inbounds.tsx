      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-black text-base tracking-tighter uppercase italic">
                {t("inbounds.title")}<span className="text-indigo-500">.</span>
              </h1>
            </div>
          ...
          <div className="flex gap-4">
            <div className="px-4 py-2 bg-card border border-line rounded-2xl flex items-center gap-3 shadow-sm">
              <div className="w-1.5 h-1.5 rounded-full bg-indigo-500 shadow-[0_0_8px_rgba(99,102,241,0.6)]" />
              <span className="text-[10px] font-black text-base uppercase tracking-widest text-muted">
                {t("inbounds.total")}: <span className="text-indigo-500">{inbounds.length}</span>
              </span>
            </div>
          </div>
        </header>
        ...
        {loading ? (
          ...
          <div className="bg-red-500/5 border border-red-500/20 text-red-400 p-8 rounded-[2.5rem] flex items-center gap-4 animate-shake">
            ...
            <div className="flex flex-col">
              <span className="font-black uppercase text-xs tracking-widest italic">System Error</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            ...
            <button 
              ...
              className="bg-card/30 border-2 border-dashed border-line rounded-[2.5rem] flex flex-col items-center justify-center p-8 group cursor-pointer hover:border-indigo-500/50 hover:bg-indigo-500/5 transition-all min-h-[220px] relative overflow-hidden active:scale-[0.98] shadow-sm"
            >
              ...
              <div key={inbound.id} className="bg-main rounded-[2.5rem] border border-line shadow-sm hover:border-indigo-500/30 hover:shadow-2xl hover:shadow-indigo-500/5 transition-all relative group overflow-hidden flex flex-col min-h-[220px]">
                <div className="p-7 flex-1">
                  <div className="flex justify-between items-start mb-6">
                    <div className={`p-3 rounded-2xl transition-all duration-500 ${inbound.is_running_in_xray ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-900/20' : 'bg-card text-muted'}`}>
                    ...
                    <div className="flex gap-2 lg:opacity-0 lg:group-hover:opacity-100 lg:translate-x-2 lg:group-hover:translate-x-0 transition-all duration-300">
                      <button 
                        ...
                        className="p-2.5 bg-card text-muted hover:text-indigo-500 hover:bg-indigo-500/5 rounded-xl transition-all"
                      >
                        <Settings2 size={14} />
                      </button>
                      <button 
                        onClick={(e) => handleDelete(e, inbound.id, inbound.tag)}
                        className="p-2.5 bg-card text-muted hover:text-red-500 hover:bg-red-500/5 rounded-xl transition-all"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
                  ...
                  <div className="space-y-1 mb-6">
                    ...
                    <div className="flex items-center gap-2 text-[9px] font-black text-indigo-500 uppercase tracking-widest">
                      <Globe size={10} />
                      {inbound.protocol || t("inbounds.protocol")}
                    </div>
                  </div>

                  <div className="grid grid-cols-2 gap-4 pt-5 border-t border-line/50">
                    <div>
                      <p className="text-[8px] font-black text-muted uppercase tracking-widest mb-1">{t("inbounds.port")}</p>
                      <p className="text-lg font-mono font-black text-base leading-none">{inbound.port}</p>
                    </div>
                    <div className="text-right">
                      <p className="text-[8px] font-black text-muted uppercase tracking-widest mb-1">{t("inbounds.clients")}</p>
                ...
                {/* Status Bar */}
                <div className={`py-3 px-7 text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-between transition-colors ${inbound.is_active ? 'bg-emerald-500/5 text-emerald-500 border-t border-emerald-500/10' : 'bg-red-500/5 text-red-500 border-t border-red-500/10'}`}>
                  <div className="flex items-center gap-2">
                    <div className={`w-1.5 h-1.5 rounded-full ${inbound.is_active ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
                    {inbound.is_active ? t("inbounds.statusActive") : t("inbounds.statusDisabled")}
                  </div>
                  <span className="opacity-40 text-[8px] italic">Inbound Core</span>
                </div>
              </div>
            ))}
          </div>
