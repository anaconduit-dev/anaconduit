...
export default function OutboundsPage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [outData, rulesData] = await Promise.all([
        getOutbounds(),
        getRoutingRules()
      ]);
  ...
  const handleDeleteRule = async (id: number) => {
    const ok = await confirm({ 
      ...
      message: "Трафик перестанет маршрутизироваться по этим условиям.", 
      type: 'danger' 
    });
    if (ok) {
    ...
    <div className="p-8 h-full overflow-y-auto custom-scrollbar bg-main">
      <div className="max-w-7xl mx-auto">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 gap-4">
            ...
            <span className="text-[10px] font-black text-muted uppercase tracking-[0.3em]">Loading Engine...</span>
          </div>
        ) : (
          <div className="animate-fadeIn">
            {activeTab === 'exits' ? (
                  ...
                  <div key={out.id} className="bg-card/40 rounded-[2.5rem] border border-line shadow-sm relative group overflow-hidden flex flex-col min-h-[240px]">
                    <div className="p-7 flex-1">
                      ...
                      <div className="mt-5 pt-5 border-t border-line/50">
                        <p className="text-[8px] font-black text-muted uppercase tracking-widest mb-1 opacity-50">Target / Description</p>
                        <p className="text-xs font-mono font-bold text-base truncate opacity-80 italic">
                          {out.settings?.servers?.[0]?.address || out.description || 'Direct System'}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className={`py-3 px-7 text-[9px] font-black uppercase tracking-[0.2em] flex items-center justify-between ${out.is_active ? 'bg-emerald-500/5 text-emerald-500' : 'bg-red-500/5 text-red-500 opacity-60'}`}>
                      ...
                      {out.is_default && <span className="text-indigo-500 text-[8px] italic">● Default Gateway</span>}
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="max-w-5xl space-y-4">
                ...
                >
                  <PlusCircle size={20} />
                  <span className="text-[10px] font-black uppercase tracking-widest">Add Traffic Distribution Rule</span>
                </button>

                {rules.length === 0 ? (
                  <div className="text-center py-20 bg-card/20 rounded-[2.5rem] border border-line">
                    <p className="text-xs font-black text-muted uppercase tracking-widest italic opacity-40">No custom rules defined</p>
                  </div>
                ) : (
                  rules.map((rule) => (
                    <div 
                    ...
                    >
                      <div className="bg-main rounded-2xl p-4 min-w-[75px] text-center border border-line shadow-inner">
                        <span className="text-[8px] font-black text-muted uppercase block mb-1 opacity-50 tracking-tighter">Priority</span>
                      ...
                      <div className="flex-1 space-y-3">
                        <div className="flex flex-wrap gap-2">
                          ...
                          {!rule.domain && !rule.ip && !rule.port && (
                            <span className="text-[9px] font-black text-muted/40 uppercase italic tracking-widest">Global Catch-all</span>
                          )}
                        </div>
                        {rule.description && (
                      ...
                      <div className="flex items-center gap-4 pl-6 border-l border-line">
                        <div className="text-right min-w-[100px]">
                          <p className="text-[8px] font-black text-muted uppercase tracking-[0.2em] mb-1 opacity-40">Forwarding To</p>
      ...
      {isEditOutboundOpen && selectedOutbound && (
        <EditOutboundModal 
          ...
          outbound={selectedOutbound} // Передаем объект, а не ID
          key={selectedOutbound.id}    // Ключ заставит React пересоздать модалку с новыми данными
          onClose={() => { 
      ...
      {isEditRuleOpen && selectedRule && (
        <EditRoutingRuleModal 
          ...
          rule={selectedRule}
          outbounds={outbounds} // Передаем список шлюзов для выпадающего списка
          key={selectedRule.id}
          onClose={() => {
