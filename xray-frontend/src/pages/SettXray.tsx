...
export default function SettXray() {
  ...
  const loadVersions = useCallback(async () => {
    ...
    try {
      ...
      toast.error(t("settXray.errorLoadVersion"));
    } finally {
  ...
  const handleInstall = async (version: string) => {
    ...
    const isConfirmed = await confirm({
      title: t("settXray.version") || "Xray Core",
      message: t("settXray.installVersion", { 
        version: version, 
        currentVersion: xrayStatus.version || "???" 
      }),
      type: 'info', // Исправлено с 'warning' на 'info'
      confirmText: t("common.confirm") || "Install",
    ...
    toast.promise(installXrayVersion(version), {
      loading: `Installing Xray ${version}...`,
      success: () => {
        setTimeout(() => {
          refreshStatus();
          setIsInstalling(false);
        }, 5000);
        return t("settXray.installSuccess", { version });
      },
      error: () => {
        setIsInstalling(false);
        return t("settXray.installError", { version });
      }
    });
  };

  const runAction = async (actionName: string, actionFn: () => Promise<any>, needsConfirm = false) => {
    if (needsConfirm) {
      const isConfirmed = await confirm({
        title: actionName,
        message: t("common.areYouSure"),
        type: 'info',
        confirmText: t("settXray.confirm"),
        cancelText: t("settXray.cancel")
      });
      if (!isConfirmed) return;
    }
    ...
    toast.promise(actionFn(), {
      ...
      success: () => {
        ...
        return t("settXray.action", { actionName });
      },
      error: () => {
        setLoadingAction(null);
        return t("settXray.actionError", { actionName });
      }
    });
  };
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-black text-base tracking-tighter uppercase italic">
                {t("layout.xrayCore")}<span className="text-indigo-500">.</span>
              </h1>
            </div>
              ...
              <div className="p-10">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  ...
                  >
                    ...
                    <span className="font-black text-[10px] text-base uppercase tracking-widest group-hover:text-emerald-500 transition-colors">{t("settXray.run")}</span>
                  </button>

                  <button
                    onClick={() => runAction('stop', stopXray, true)} // Исправлено: передаем true
                    disabled={!!loadingAction || xrayStatus.status !== 'running'}
                  ...
                  >
                    ...
                    <span className="font-black text-[10px] text-base uppercase tracking-widest group-hover:text-red-500 transition-colors">{t("settXray.stop")}</span>
                  </button>

                  <button
                    onClick={() => runAction('restart', restartXray, true)} // Исправлено: передаем true
                  ...
                  >
                    ...
                    <span className="font-black text-[10px] text-base uppercase tracking-widest group-hover:text-indigo-500 transition-colors">{t("settXray.restart")}</span>
                  </button>
                </div>
                {/* Блок с message.text удален, так как используем тосты */}
              </div>
            </div>
            ...
            <div className="bg-card/50 border border-line rounded-[2.5rem] overflow-hidden flex flex-col h-[650px] shadow-sm">
              <div className="p-6 border-b border-line bg-card/30 flex items-center justify-between sticky top-0 z-10">
                <div className="flex items-center gap-3">
                  ...
                  <h3 className="text-[10px] font-black text-base uppercase tracking-widest">{t("settXray.version")}</h3>
                </div>
                <button 
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-3 custom-scrollbar">
                {isLoadingVersions ? (
                  <div className="flex flex-col items-center justify-center h-full text-muted gap-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">{t("settXray.synchronization")}</span>
                  </div>
                ) : (
                  versions.map((v) => {
