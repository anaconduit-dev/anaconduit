...
class XrayService:
    ...
    async def ensure_base_config(self):
        base_config = {
            "log": {"loglevel": "info"},
            "stats": {},  # 1. Активируем модуль статистики
            "api": {
                "tag": "api",
                "services": ["HandlerService", "StatsService", "LoggerService"]
            },
            "policy": {   # 2. Глобальное разрешение на сбор данных
                "levels": {
                    "0": {
    ...
    async def install(self, version: str) -> Dict[str, Any]:
        ...
        # Формируем словарь портов. 
        # Ключ — порт внутри контейнера, значение — порт на твоем хосте.
        exposed_ports = {
            f"{self.API_PORT}/tcp": self.API_PORT,
            "1080/tcp": 1080, # Добавляем порт для SOCKS5
            "1080/udp": 1080  # UDP часто нужен для прокси (игры, звонки)
        }

        container = await self.docker.run_container(
            name=self.CONTAINER_NAME,
            image=image,
            ports=exposed_ports, # Передаем обновленный словарь
            volumes={
                self.host_xray_dir: {
                    "bind": "/etc/xray",
                    "mode": "rw",
